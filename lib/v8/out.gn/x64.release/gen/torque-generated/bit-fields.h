...
#define DEFINE_TORQUE_GENERATED_SHARED_FUNCTION_INFO_FLAGS() \
  ...
  using PrivateNameLookupSkipsOuterClassBit = base::BitField<bool, 30, 1, uint32_t>; \
...
#define DEFINE_TORQUE_GENERATED_SHARED_FUNCTION_INFO_FLAGS2() \
  ...
  using HasOptimizedAtLeastOnceBit = base::BitField<bool, 2, 1, uint8_t>; \
  using MayHaveCachedCodeBit = base::BitField<bool, 3, 1, uint8_t>; \
  enum Flag { \
    kNone = 0, \
    kClassScopeHasPrivateBrand = 1 << 0, \
    kHasStaticPrivateMethodsOrAccessors = 1 << 1, \
    kHasOptimizedAtLeastOnce = 1 << 2, \
    kMayHaveCachedCode = 1 << 3, \
  }; \
  using Flags = base::Flags<Flag>; \
