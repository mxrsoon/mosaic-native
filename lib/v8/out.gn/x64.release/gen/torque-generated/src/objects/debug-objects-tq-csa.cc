...
TorqueStructSlice_CoverageInfoSlot_MutableReference_CoverageInfoSlot_0 FieldSliceCoverageInfoSlots_0(compiler::CodeAssemblerState* state_, TNode<CoverageInfo> p_o) {
  ...
  if (block0.is_used()) {
    ...
    std::tie(tmp5, tmp6, tmp7) = NewMutableSlice_CoverageInfoSlot_0(state_, TNode<Object>{p_o}, TNode<IntPtrT>{tmp4}, TNode<IntPtrT>{tmp3}).Flatten();
    ca_.Goto(&block2);
  }

    ca_.Bind(&block2);
  return TorqueStructSlice_CoverageInfoSlot_MutableReference_CoverageInfoSlot_0{TNode<Object>{tmp5}, TNode<IntPtrT>{tmp6}, TNode<IntPtrT>{tmp7}, TorqueStructUnsafe_0{}};
}
