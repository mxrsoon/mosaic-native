...
void VerifiedUnreachable_0(compiler::CodeAssemblerState* state_) {
  ...
  if (block0.is_used()) {
    ...
    CodeStubAssembler(state_).StaticAssert(TNode<BoolT>{tmp0}, "static_assert(false) at src/builtins/base.tq:1670:3");
    CodeStubAssembler(state_).Unreachable();
  }
}
...
TF_BUILTIN(CheckNumberInRange, CodeStubAssembler) {
  ...
  TNode<Context> parameter0 = UncheckedParameter<Context>(Descriptor::kContext);
  USE(parameter0);
  TNode<Number> parameter1 = UncheckedParameter<Number>(Descriptor::ParameterIndex<0>());
  USE(parameter1);
  TNode<Number> parameter2 = UncheckedParameter<Number>(Descriptor::ParameterIndex<1>());
  USE(parameter2);
  ...
  if (block5.is_used()) {
    ...
    ca_.Branch(phi_bb5_6, &block6, std::vector<Node*>{}, &block7, std::vector<Node*>{});
  }
  ...
  if (block8.is_used()) {
    ...
    ca_.Branch(phi_bb8_6, &block1, std::vector<Node*>{}, &block2, std::vector<Node*>{});
  }
