...
TorqueStructFrameWithArgumentsInfo_0 GetFrameWithArgumentsInfo_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context) {
  ...
  compiler::CodeAssemblerParameterizedLabel<RawPtrT, BInt> block13(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
    ca_.Goto(&block0);
  ...
  if (block7.is_used()) {
    ca_.Bind(&block7);
    tmp12 = LoadCallerFromFrame_0(state_, TNode<RawPtrT>{tmp1});
    compiler::CodeAssemblerLabel label14(&ca_);
    tmp13 = Cast_ArgumentsAdaptorFrame_0(state_, TNode<Context>{p_context}, TNode<RawPtrT>{tmp12}, &label14);
    ca_.Goto(&block11);
    if (label14.is_used()) {
      ca_.Bind(&label14);
      ca_.Goto(&block12);
  ...
  if (block11.is_used()) {
    ca_.Bind(&block11);
    tmp15 = LoadLengthFromAdapterFrame_0(state_, TNode<Context>{p_context}, TNode<RawPtrT>{tmp13});
    tmp16 = Convert_bint_Smi_0(state_, TNode<Smi>{tmp15});
  ...
  if (block1.is_used()) {
    ...
    ca_.Goto(&block13, phi_bb1_1, phi_bb1_2);
  }

  TNode<RawPtrT> phi_bb13_1;
  TNode<BInt> phi_bb13_2;
    ca_.Bind(&block13, &phi_bb13_1, &phi_bb13_2);
  return TorqueStructFrameWithArgumentsInfo_0{TNode<RawPtrT>{phi_bb13_1}, TNode<BInt>{phi_bb13_2}, TNode<BInt>{tmp9}};
}
