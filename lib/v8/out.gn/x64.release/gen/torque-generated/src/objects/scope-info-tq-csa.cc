...
TNode<Uint32T> LoadScopeInfoFlags_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<ScopeInfo> p_scopeInfo) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    std::tie(tmp0, tmp1, tmp2) = FieldSliceFixedArrayObjects_0(state_, TNode<FixedArray>{p_scopeInfo}).Flatten();
    tmp3 = FromConstexpr_intptr_constexpr_int32_0(state_, ScopeInfo::Fields::kFlags);
  ...
  if (block6.is_used()) {
    ...
    tmp13 = Convert_ScopeFlags_Smi_0(state_, TNode<Smi>{tmp12});
    ca_.Goto(&block9);
  }
