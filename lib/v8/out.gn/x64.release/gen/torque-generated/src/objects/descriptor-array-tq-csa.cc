...
TorqueStructSlice_DescriptorEntry_MutableReference_DescriptorEntry_0 FieldSliceDescriptorArrayDescriptors_0(compiler::CodeAssemblerState* state_, TNode<DescriptorArray> p_o) {
  ...
  if (block0.is_used()) {
    ...
    tmp2 = CodeStubAssembler(state_).LoadReference<Uint16T>(CodeStubAssembler::Reference{p_o, tmp1});
    tmp3 = Convert_intptr_uint16_0(state_, TNode<Uint16T>{tmp2});
    tmp4 = FromConstexpr_intptr_constexpr_int31_0(state_, 16);
    std::tie(tmp5, tmp6, tmp7) = NewMutableSlice_DescriptorEntry_0(state_, TNode<Object>{p_o}, TNode<IntPtrT>{tmp4}, TNode<IntPtrT>{tmp3}).Flatten();
    ca_.Goto(&block2);
  }

    ca_.Bind(&block2);
  return TorqueStructSlice_DescriptorEntry_MutableReference_DescriptorEntry_0{TNode<Object>{tmp5}, TNode<IntPtrT>{tmp6}, TNode<IntPtrT>{tmp7}, TorqueStructUnsafe_0{}};
}
