...
template <typename Impl>
Handle<WasmCapiFunctionData> TorqueGeneratedFactory<Impl>::NewWasmCapiFunctionData(Address call_target, Handle<Foreign> embedder_data, Handle<Code> wrapper_code, Handle<PodArray<wasm::ValueType>> serialized_signature, AllocationType allocation_type) {
 int size = TorqueGeneratedWasmCapiFunctionData<WasmCapiFunctionData, HeapObject>::SizeFor();
  Map map = factory()->read_only_roots().wasm_capi_function_data_map();  HeapObject result =
    factory()->AllocateRawWithImmortalMap(size, allocation_type, map);
    WriteBarrierMode write_barrier_mode =
       allocation_type == AllocationType::kYoung
       ? SKIP_WRITE_BARRIER : UPDATE_WRITE_BARRIER;
  Handle<WasmCapiFunctionData> result_handle(WasmCapiFunctionData::cast(result), factory()->isolate());
  result_handle->TorqueGeneratedClass::set_call_target(call_target);
  result_handle->TorqueGeneratedClass::set_embedder_data(*embedder_data, write_barrier_mode);
  result_handle->TorqueGeneratedClass::set_wrapper_code(*wrapper_code, write_barrier_mode);
  result_handle->TorqueGeneratedClass::set_serialized_signature(*serialized_signature, write_barrier_mode);
  return result_handle;
}

template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) Handle<WasmCapiFunctionData>TorqueGeneratedFactory<Factory>::NewWasmCapiFunctionData(Address call_target, Handle<Foreign> embedder_data, Handle<Code> wrapper_code, Handle<PodArray<wasm::ValueType>> serialized_signature, AllocationType allocation_type);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) Handle<WasmCapiFunctionData>TorqueGeneratedFactory<LocalFactory>::NewWasmCapiFunctionData(Address call_target, Handle<Foreign> embedder_data, Handle<Code> wrapper_code, Handle<PodArray<wasm::ValueType>> serialized_signature, AllocationType allocation_type);
