...
USE(parameter2);
  ...
  if (block20.is_used()) {
    ...
    ca_.Branch(tmp34, &block25, std::vector<Node*>{}, &block26, std::vector<Node*>{});
  }
  ...
  if (block25.is_used()) {
    ...
    ca_.Branch(tmp45, &block28, std::vector<Node*>{}, &block29, std::vector<Node*>{});
  }
...
void InitContextSlot_PromiseAnyRejectElementContext_PromiseAnyRejectElementContext_Smi_constexpr_int31_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<IntPtrT> p_index, int31_t p_value) {
  ...
  if (block0.is_used()) {
    ...
    ca_.Branch(tmp6, &block17, std::vector<Node*>{}, &block18, std::vector<Node*>{});
  }
